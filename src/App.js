...
const IMGS = {
  ...
  "Chilli Birds Eye":"https://i.ibb.co/PJSwxT1/Capsicum-Birds-Eye.jpg",
...
export default function App() {
  ...
  const adminFiltered = useMemo(() => products.filter(p =>
    adminFilter==="All" || (adminFilter==="outofstock" && p.outOfStock) || (adminFilter==="hidden" && !p.stock) || (adminFilter==="live" && p.stock && !p.outOfStock)
  ), [products, adminFilter]);
    ...
    <div style={{fontFamily:"Georgia,serif",minHeight:"100vh",background:C.offwhite,display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{...boxStyle,textAlign:"center",maxWidth:480}}>
        ...
        <div style={{background:C.parchment,border:"1px solid "+C.border,borderRadius:10,padding:"1rem",marginBottom:"1rem",fontSize:13,color:C.brown,lineHeight:1.8}}>
          <strong>What happens next?</strong><br/>Your WhatsApp message has been sent with your order details. We will confirm and arrange your <strong>Pudo locker delivery</strong> within 1 business day.<br/><br/>Questions? Call us on <strong>{PHONE}</strong>
        </div>
        <div style={{...pudoNote,marginBottom:"1rem",fontSize:13}}>You will receive an SMS from Pudo when your seeds are ready for collection.</div>
        <button style={{...bGreen,width:"100%",padding:12,marginBottom:8}} onClick={()=>{window.history.replaceState({},"","/");setPaymentStatus(null);}}>Continue shopping</button>
    ...
    <div style={{fontFamily:"Georgia,serif",minHeight:"100vh",background:C.offwhite,color:C.text}}>
      ...
      {view==="store" && <div>
        ...
        <div style={{background:C.parchment,borderBottom:"1px solid "+C.border,borderTop:"1px solid "+C.border,padding:"1rem",display:"flex",justifyContent:"center",gap:"2rem",flexWrap:"wrap"}}>
          {[["1. Browse","260+ heirloom varieties"],["2. Checkout","Pay securely via YOCO"],["3. Pudo Delivery","Collect from your nearest locker"],["4. Grow & Save","Open-pollinated seeds for life"]].map(([t,d])=>(
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(180px,1fr))",gap:12,padding:"1rem"}}>
          {filtered.map(p=>{
            ...
            return (
              <div key={p.id} style={{background:"#fff",borderRadius:10,border:"1px solid "+C.border,overflow:"hidden",display:"flex",flexDirection:"column",opacity:p.outOfStock?0.85:1}}>
                <div style={{width:"100%",height:130,background:cs.bg,overflow:"hidden",flexShrink:0,position:"relative",display:"flex",alignItems:"center",justifyContent:"center"}}>
                  {p.image
                    ...
                    : null}
                  <span style={{fontSize:36,display:p.image?"none":"flex",position:"absolute"}}>{cs.icon}</span>
                  {p.outOfStock && (
                    <div style={{position:"absolute",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.5)",display:"flex",alignItems:"center",justifyContent:"center"}}>
                      <span style={{background:"#c00",color:"#fff",fontSize:12,fontWeight:700,padding:"4px 12px",borderRadius:20,letterSpacing:"0.5px"}}>OUT OF STOCK</span>
                    </div>
                  )}
                </div>
        ...
        !adminAuth ? (
          ...
          <div style={{padding:"1.25rem",maxWidth:900,margin:"0 auto"}}>
            ...
            <div style={{display:"flex",gap:6,marginBottom:"1rem",flexWrap:"wrap"}}>
              {[["All","All products"],["live","Live"],["outofstock","Out of stock"],["hidden","Hidden"]].map(([val,label])=>(
                <button key={val} style={{...bParch,padding:"5px 12px",fontSize:12,background:adminFilter===val?C.darkGreen:C.parchment,color:adminFilter===val?"#fff":C.brown}} onClick={()=>setAdminFilter(val)}>{label} ({val==="All"?products.length:val==="outofstock"?products.filter(p=>p.outOfStock).length:val==="hidden"?products.filter(p=>!p.stock).length:products.filter(p=>p.stock&&!p.outOfStock).length})</button>
              ))}
            </div>
            ...
            {adminFiltered.map(p=>(
              <div key={p.id} style={{background:"#fff",border:"1px solid "+C.border,borderRadius:8,padding:"9px 12px",display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                ...
                <button style={{fontSize:11,padding:"3px 8px",borderRadius:8,border:"none",cursor:"pointer",background:p.outOfStock?"#c00":"#eef5e8",color:p.outOfStock?"#fff":C.midGreen,fontWeight:600,flexShrink:0}} onClick={()=>setProducts(ps=>ps.map(x=>x.id===p.id?{...x,outOfStock:!x.outOfStock}:x))}>
            ...
            {step===2&&(
              <div>
                ...
                <div style={{background:"#fff8e8",border:"1px solid #e8d08a",borderRadius:8,padding:"0.8rem",fontSize:12,color:C.brown,marginBottom:"0.9rem",lineHeight:1.7}}>
                  Clicking Pay now opens a secure YOCO payment page with the exact amount pre-filled. Your order summary will also be sent to us on WhatsApp.
                </div>
