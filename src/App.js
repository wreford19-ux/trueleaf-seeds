...
const ALL_CATS = ["Crop Cover","Flower","Herb","Lawn","Microgreen","Sprouts","Tree","Vegetable"];
...
const IMGS = {
  ...
  // 10 NEW PRODUCTS
  "California Fan Palm":"https://i.ibb.co/1Y6TWCNt/California-Fan-Palm-TPCF.png",
  "Horned Thorn":"https://i.ibb.co/tPD3YYwT/Horned-Thorn-THTV.png",
  "Kiepersol":"https://i.ibb.co/tTPc4W9w/Kiepersol-TKCP.png",
  "Manoko":"https://i.ibb.co/s951gnWh/Manoko-PPMA.png",
  "Mulamje":"https://i.ibb.co/CpXLNpp1/Mulamje-PPMU.png",
  "Mulato":"https://i.ibb.co/ZR7t8Hpn/Mulato-PPMu.png",
  "Natal Plum":"https://i.ibb.co/qYYxKXg9/Natal-Plum-TSNP.png",
  "Palm Mexican Fan Palm":"https://i.ibb.co/SwxcFW76/Palm-Mexican-Fan-Palm-TPMF.png",
  "Searsia Lancea":"https://i.ibb.co/B2V7YCs6/Searsia-Lancea-TSLR.png",
  "Senqu":"https://i.ibb.co/CpRhVnYN/Senqu-PPSE.png",
};
...
const RAW=[
  ...
  // 10 NEW
  ["California Fan Palm","Tree"],["Horned Thorn","Tree"],["Kiepersol","Tree"],["Manoko","Vegetable"],["Mulamje","Vegetable"],["Mulato","Vegetable"],["Natal Plum","Tree"],["Palm Mexican Fan Palm","Tree"],["Searsia Lancea","Tree"],["Senqu","Vegetable"],
];
...
const CS={Vegetable:{bg:"#e8f5ee",color:"#2d6a3f",icon:"🥦"},Flower:{bg:"#fdf0f8",color:"#8b3a72",icon:"🌸"},Herb:{bg:"#fdf8e8",color:"#7a5c00",icon:"🌿"},Tree:{bg:"#eef5e8",color:"#2d4a1e",icon:"🌳"},Microgreen:{bg:"#e8f8f8",color:"#0a6060",icon:"🌱"},Sprouts:{bg:"#e8eef8",color:"#1a3a7a",icon:"🫘"},"Crop Cover":{bg:"#f5efe8",color:"#6b3a00",icon:"🌾"},Lawn:{bg:"#eef5e0",color:"#3a6000",icon:"🍀"}};
...
const BLANK_PRODUCT={name:"",category:"Vegetable",cost:25,price:45,stock:true,outOfStock:false,image:""};
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    try{const s=localStorage.getItem("tl_orders");if(s)setOrders(JSON.parse(s));}catch{}
    try{const s=localStorage.getItem("tl_products");if(s)setProducts(JSON.parse(s));}catch{}
  },[]);

  const saveOrders=o=>{setOrders(o);try{localStorage.setItem("tl_orders",JSON.stringify(o));}catch{}};
  const saveProducts=p=>{setProducts(p);try{localStorage.setItem("tl_products",JSON.stringify(p));}catch{}};
  const showToast=msg=>{setToast(msg);setTimeout(()=>setToast(""),2500);};

  const storeFiltered=useMemo(()=>products.filter(p=>(cat==="All"||p.category===cat)&&p.name.toLowerCase().includes(search.toLowerCase())&&p.stock),[products,cat,search]);
  const adminFiltered=useMemo(()=>products.filter(p=>(prodCat==="All"||p.category===prodCat)&&p.name.toLowerCase().includes(prodSearch.toLowerCase())),[products,prodCat,prodSearch]);
  ...
  const sendEmail=async order=>{
    try{
      await fetch("https://api.emailjs.com/api/v1.0/email/send",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({service_id:EMAILJS_SERVICE,template_id:EMAILJS_TEMPLATE,user_id:EMAILJS_KEY,template_params:{order_id:order.id,date:order.date,customer_name:order.customer.name,customer_phone:order.customer.phone,customer_email:order.customer.email,address:order.customer.street+", "+order.customer.suburb+", "+order.customer.city+", "+order.customer.province+" "+order.customer.postal,items:order.items.map(x=>x.qty+"x "+x.name+" @ R"+x.price+" = R"+(x.price*x.qty).toFixed(2)).join(", "),seeds_total:order.seedsTotal.toFixed(2),order_total:order.total.toFixed(2)}})});
  ...
  const handlePay=async()=>{
    ...
    try{
      ...
      if(data.redirectUrl){sendEmail(newOrder);window.location.href=data.redirectUrl;}
  ...
  const saveProduct=()=>{
    ...
    if(isNewProduct){
      const n={...editData,id:Date.now(),price:Number(editData.price),cost:Number(editData.cost)};
        ...
        <div style={{padding:"1.25rem",maxWidth:960,margin:"0 auto"}}>
          ...
          {adminTab==="products"&&<div>
            ...
            <div style={{display:"flex",gap:8,marginBottom:"1rem",flexWrap:"wrap",alignItems:"center"}}>
              <input style={{...iS,flex:1,minWidth:160}} placeholder="Search products..." value={prodSearch} onChange={e=>setProdSearch(e.target.value)}/>
      ...
      {editId&&(
        <div style={mS} onClick={e=>e.target===e.currentTarget&&setEditId(null)}>
          <div style={bxS}>
            ...
            <select style={{...iS,marginBottom:12}} value={editData.category||"Vegetable"} onChange={e=>setEditData(d=>({...d,category:e.target.value}))}>
